...
AnalogueMeter::AnalogueMeter(TFT_eSPI* tft)
{
    this->tft = tft;
}
...
// #########################################################################
void AnalogueMeter::plotNeedle(String label, int value, byte ms_delay)
{
    ...
    while (!(value == old_analog))
    {
        ...
        tft->setTextColor(TFT_BLACK, TFT_WHITE);
        tft->drawCentreString(label, M_SIZE * 120, M_SIZE * 75, 4); // // Comment out to avoid font 4
...
// #########################################################################
void AnalogueMeter::analogMeter()
{
    ...
    for (int i = -50; i < 51; i += 5)
    {
        ...
        if (i % 25 == 0)
        {
            ...
            case -1:
                ...
                break;
            case 0:
                tft->drawCentreString("50", x0, y0, 1);
                break;
            case 1:
    ...
    tft->drawString("%", M_SIZE * (3 + 230 - 40), M_SIZE * (119 - 20), 2); // Units at bottom right
    tft->drawRect(1, M_SIZE * 3, M_SIZE * 236, M_SIZE * 126, TFT_BLACK);   // Draw bezel line

    plotNeedle("null", 0, 0); // Put meter needle at 0
}